...
struct FFDrawView : UIViewControllerRepresentable {
    // Init your ViewController
    
    func makeUIViewController(context: Context) -> DrawViewController {
        let storyBoard = UIStoryboard(name: "Main", bundle: nil)
        guard let vc = storyBoard.instantiateInitialViewController() else {
            return DrawViewController()
        }
        return vc as! DrawViewController
    }
    
    // Tbh no idea what to do here
    func updateUIViewController(_ uiViewController: DrawViewController, context: Context) {
